...
const FiltersSidebar = ({ categories, cities, city, maxPrice, onCityChange, onToggleCategory, onMaxPriceChange, onReset }) => {
  return (
    <aside className="marketplace-filters" aria-label="Filters">
      ...
      <div className="marketplace-filters__group">
        <label className="marketplace-filters__label" htmlFor="marketplace-city">City</label>
        <div className="marketplace-filters__select">
          <select id="marketplace-city" value={city} onChange={(e) => onCityChange(e.target.value)}>
        ...
        <div className="marketplace-filters__options">
          {categories.map((cat) => (
            <label key={cat.label} className="marketplace-filters__option">
              <input type="checkbox" checked={cat.checked} onChange={() => onToggleCategory(cat.label)} />
              <span>{cat.label}</span>
            </label>
          ))}
        </div>
      ...
      <div className="marketplace-filters__group">
        <span className="marketplace-filters__label">Max Price: {maxPrice} DH / unit</span>
        <input
          type="range"
          min={5}
          max={50}
        ...
        />
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', opacity: 0.6 }}>
          <span>5 DH</span><span>50 DH</span>
        </div>
      </div>
    </aside>
  );
